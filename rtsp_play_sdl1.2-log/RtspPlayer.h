...
private:
    //SDL
    bool InitSDL_component(int Pos_x, int Pos_y, int Width, int Height);
    void DeInitSDL_component();
    ...
    int EncodeOneFrameToJpeg(AVFrame *pSrcFrame, AVPixelFormat srcFormat, unsigned char* DestBuffer, int* bufferLength, int destWidth, int destHeight);
    int EncodeOneFrameToBMp(AVFrame *pSrcFrame, AVPixelFormat srcFormat, unsigned char* DestBuffer, int* bufferLength, int destWidth, int destHeight);
    static void encode(AVCodecContext *enc_ctx, AVFrame *frame, AVPacket *pkt, unsigned char *outfile, int* piLength);
